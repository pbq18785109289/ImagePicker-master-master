...
public class VideoPreviewActivity extends VideoPreviewBaseActivity implements VideoPicker.OnVideoSelectedListener, View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ImageItem item = mVideoItems.get(mCurrentPosition);
        ...
        mCbCheck.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ImageItem videoItem = mVideoItems.get(mCurrentPosition);
                ...
                if (mCbCheck.isChecked() && selectedVideos.size() >= selectLimit) {
                    ...
                    videoPicker.addSelectedImageItem(mCurrentPosition, videoItem, mCbCheck.isChecked());
                }
            }
        });
    }
    ...
    public void onVideoSelected(int position, ImageItem item, boolean isAdd) {
        ...
        if (mCbOrigin.isChecked()) {
            long size = 0;
            for (ImageItem videoItem : selectedVideos)
                size += videoItem.size;
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        ...
        if (id == R.id.cb_origin) {
            if (isChecked) {
                long size = 0;
                for (ImageItem item : selectedVideos)
                    size += item.size;
                String fileSize = Formatter.formatFileSize(this, size);
                isOrigin = true;
                mCbOrigin.setText(getString(R.string.origin_size, fileSize));
