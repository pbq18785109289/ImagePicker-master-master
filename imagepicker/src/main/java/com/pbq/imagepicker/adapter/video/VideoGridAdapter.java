...
public class VideoGridAdapter extends BaseAdapter {
    ...
    private ArrayList<ImageItem> videos;       //当前需要显示的所有的视频数据
    private ArrayList<ImageItem> mSelectedVideos; //全局保存的已经选中的视频数据
    private boolean isShowCamera;         //是否显示录像按钮
    private int mVideoSize;               //每个条目的大小
    private OnVideoItemClickListener listener;   //视频被点击的监听

    public VideoGridAdapter(Activity activity, ArrayList<ImageItem> videos) {
    ...
    public void refreshData(ArrayList<ImageItem> videos) {
        if (videos == null || videos.size() == 0) this.videos = new ArrayList<>();
        else this.videos = videos;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (itemViewType == ITEM_TYPE_CAMERA) {
            ...
            final ImageItem videoItem = getItem(position);
            /**
             * 点击图片的监听
             */
            ...
            //显示时长 转化成分秒
            double size= videoItem.size/1024/1024;
            holder.tv_timelong.setText(size+"M");
            ...
            holder.cbCheck.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (holder.cbCheck.isChecked() && mSelectedVideos.size() >= selectLimit) {
                        ...
                        videoPicker.addSelectedImageItem(position, videoItem, holder.cbCheck.isChecked());
    ...
    public interface OnVideoItemClickListener {
        void onVideoItemClick(View view, ImageItem videoItem, int position);
    }
}